...
export default function PaymentQRPage() {
  ...
  return <div className="bg-bg-canvas py-8 sm:py-12"><Container>
    ...
    <div className="mb-8"><p className="text-xs font-semibold uppercase tracking-caps text-brand-fg">Thanh toán an toàn</p><h1 className="mt-2 text-h1">Thanh toán bằng mã QR</h1><p className="mt-3 max-w-2xl text-fg-muted">Mở ứng dụng ngân hàng hoặc MoMo, quét mã và giữ nguyên nội dung chuyển khoản để cửa hàng đối soát chính xác.</p></div>
    <div className="grid gap-6 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]">
      ...
      <div className="flex flex-col items-center rounded-xl bg-brand p-6 text-center text-fg-on-brand sm:p-10"><div className="flex items-center gap-2 text-sm font-semibold uppercase tracking-caps text-fg-on-brand/80"><CreditCard className="size-4" />VietQR / MoMo</div><h2 className="mt-4 font-heading text-2xl font-semibold text-fg-on-brand">Quét mã để thanh toán</h2><p className="mt-2 max-w-sm text-sm text-fg-on-brand/80">Dùng ứng dụng ngân hàng hoặc MoMo. Sau khi chuyển, quay về đơn hàng để xem trạng thái xác nhận.</p><div className="mt-8 rounded-xl bg-fg-on-brand p-5 shadow-xl">{qrImageUrl ? <img src={qrImageUrl} alt="Mã QR thanh toán bằng ngân hàng hoặc MoMo" width="320" height="320" className="size-64 object-contain sm:size-80" /> : <div className="grid size-64 place-items-center bg-bg-inset text-sm text-fg-muted sm:size-80">Mã QR chưa sẵn sàng</div>}</div><p className="mt-6 text-sm text-fg-on-brand/80">Cần trợ giúp? <Link to="/contact" className="font-semibold text-fg-on-brand underline underline-offset-4">Liên hệ Leaf Creme</Link>.</p></div>
    </div>
  </Container></div>
}
