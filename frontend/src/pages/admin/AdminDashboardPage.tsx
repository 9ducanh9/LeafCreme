...
export default function AdminDashboardPage() {
  ...
  const { revenueData, productRevenue, bestSellers, categoryRevenue, stats, alertsSummary, loading } = useDashboardData(timeRange)
  const average = stats && stats.totalOrders ? stats.totalRevenue / stats.totalOrders : 0
  ...
  return (
    <AdminPage title="Tổng quan kinh doanh" breadcrumb={[{ label: 'Tổng quan' }]}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, 1fr)', lg: 'repeat(4, 1fr)' }, gap: 2, mb: 3 }}>
        <StatCard label="Doanh thu" value={stats ? money(stats.totalRevenue) : '—'} icon={<AttachMoneyIcon />} />
        <StatCard label="Đơn hàng" value={stats?.totalOrders ?? '—'} icon={<ShoppingCartIcon />} />
        <StatCard label="Giá trị trung bình" value={money(average)} icon={<StarIcon />} />
        <StatCard label="Sản phẩm bán chạy" value={stats?.bestSeller || '—'} icon={<StarIcon />} />
      </Box>
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '1.4fr 1fr' }, gap: 2 }}>
        <Card><CardContent><Typography variant="h6" gutterBottom>Doanh thu theo ngày</Typography>{revenueData.length ? <Stack spacing={1}>{revenueData.slice(-8).map((item) => <Box key={item.date}><Box sx={{ display: 'flex', justifyContent: 'space-between' }}><Typography variant="body2">{item.date}</Typography><Typography variant="body2" fontWeight={700}>{money(item.revenue)}</Typography></Box><LinearProgress variant="determinate" value={Math.min(100, (item.revenue / Math.max(...revenueData.map((entry) => entry.revenue), 1)) * 100)} /></Box>)}</Stack> : <Typography color="text.secondary">Chưa có dữ liệu doanh thu.</Typography>}</CardContent></Card>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(3, 1fr)' }, gap: 2, mt: 2 }}>
        <Card><CardContent><Typography variant="h6" gutterBottom>Sản phẩm theo doanh thu</Typography>{productRevenue.slice(0, 5).map((item) => <Box key={item.productName} sx={{ display: 'flex', justifyContent: 'space-between', py: 0.75 }}><Typography variant="body2">{item.productName}</Typography><Typography variant="body2" fontWeight={600}>{money(item.revenue)}</Typography></Box>)}</CardContent></Card>
        <Card><CardContent><Typography variant="h6" gutterBottom>Bán chạy</Typography>{bestSellers.slice(0, 5).map((item) => <Box key={item.productName} sx={{ display: 'flex', justifyContent: 'space-between', py: 0.75 }}><Typography variant="body2">{item.productName}</Typography><Typography variant="body2">{item.quantity} sản phẩm</Typography></Box>)}</CardContent></Card>
        <Card><CardContent><Typography variant="h6" gutterBottom>Danh mục</Typography>{categoryRevenue.slice(0, 5).map((item) => <Box key={item.category} sx={{ display: 'flex', justifyContent: 'space-between', py: 0.75 }}><Typography variant="body2">{item.category}</Typography><Typography variant="body2" fontWeight={600}>{money(item.revenue)}</Typography></Box>)}</CardContent></Card>
      </Box>
    </AdminPage>
  )
}
