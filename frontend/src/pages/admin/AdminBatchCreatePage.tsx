...
export default function AdminBatchCreatePage() {
  ...
  const submit = async () => {
    ...
    const warnings = validateBatchSoft(values)
    setWarning(Object.values(warnings)[0] || null)
  ...
  return (
    <AdminPage title="Nhập lô hàng" breadcrumb={[{ label: 'Nhập lô' }]}>
      ...
      <form onSubmit={(event) => { event.preventDefault(); void submit() }}>
        ...
        <Button type="button" onClick={reset} disabled={loading} sx={{ mr: 1 }}>Đặt lại</Button><Button type="submit" variant="contained" disabled={loading}>Tạo lô</Button>
      </form>
    </AdminPage>
  )
}
