...
export default function ProductCard({ product }: { product: Product }) {
  return <Card interactive className="group h-full p-0">
    <CardMedia ratio="square" className="bg-bg-subtle"><img src={product.hinh_anh_url ? getImageUrl(product.hinh_anh_url) : FALLBACK_IMAGE.product} alt={product.ten} width="600" height="600" loading="lazy" className="size-full object-contain p-1.5 transition-transform duration-slow group-hover:scale-[1.02]" onError={(event) => { event.currentTarget.src = FALLBACK_IMAGE.product }} /><div className="absolute left-3 top-3">{product.danh_muc && <Badge variant="brand">{product.danh_muc}</Badge>}</div></CardMedia>
