...
export default function ProductCard({ product }: { product: Product }) {
  return <Card interactive className="group p-0">
    ...
    <CardBody className="gap-2"><CardTitle><Link to={`/products/${product.sanpham_id}`} className="outline-none after:absolute after:inset-0 after:z-raised after:content-['']">{product.ten}</Link></CardTitle><p className="line-clamp-2 text-sm text-fg-muted">{product.mo_ta || 'Một món bánh thủ công từ Leaf Crème.'}</p></CardBody>
