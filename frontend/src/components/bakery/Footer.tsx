...
const links = [
  ...
  { to: '/privacy-policy', label: 'Quyền riêng tư' },
  { to: '/data-deletion', label: 'Xoá dữ liệu' },
]
...
export default function Footer() {
  return <footer className="border-t border-border-subtle bg-bg-subtle">
    <Container className="py-12 sm:py-16">
      <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-[1.4fr_0.75fr_0.75fr_1fr]">
        <div><Link to="/" className="font-heading text-2xl font-semibold text-fg-strong">Leaf Creme</Link><p className="mt-3 max-w-sm text-sm leading-relaxed text-fg-muted">Những chiếc bánh nhỏ cho những khoảnh khắc đáng nhớ — làm thủ công từ căn bếp Sài Gòn.</p></div>
        <div><h2 className="text-sm font-semibold uppercase tracking-caps text-fg-strong">Khám phá</h2><nav aria-label="Liên kết cuối trang" className="mt-4 flex flex-col items-start gap-3 text-sm">{primaryLinks.map((link) => <Link key={link.to} to={link.to} className="text-fg-muted transition-colors hover:text-brand-fg">{link.label}</Link>)}</nav></div>
        <nav aria-label="Chính sách và quyền riêng tư" className="flex flex-col items-start gap-3 pt-7 text-sm">{secondaryLinks.map((link) => <Link key={link.to} to={link.to} className="text-fg-muted transition-colors hover:text-brand-fg">{link.label}</Link>)}</nav>
      ...
      <div className="mt-10 border-t border-border-subtle pt-6 text-sm text-fg-subtle"><p>© {new Date().getFullYear()} Leaf Creme. Làm bằng sự tử tế.</p></div>
    </Container>
  </footer>
}
