...
export default function Footer() {
  return <footer className="border-t border-border-subtle bg-bg-subtle">
    <Container className="py-12 sm:py-16">
      ...
      <div className="mt-10 flex flex-col gap-3 border-t border-border-subtle pt-6 text-sm text-fg-subtle sm:flex-row sm:items-center sm:justify-between"><p>© {new Date().getFullYear()} Leaf Crème. Làm bằng sự tử tế.</p><nav aria-label="Liên kết pháp lý" className="flex gap-4 text-xs"><Link to="/privacy-policy" className="hover:text-brand-fg">Quyền riêng tư</Link><Link to="/data-deletion" className="hover:text-brand-fg">Xoá dữ liệu</Link></nav></div>
    </Container>
  </footer>
}
